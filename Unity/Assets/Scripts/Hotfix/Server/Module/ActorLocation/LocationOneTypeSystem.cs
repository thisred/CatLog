namespace ET.Server
{
    ...
    public static partial class LocationOneTypeSystem
    {
        ...
        {
            int coroutineLockType = (self.LocationType << 16) | CoroutineLockType.Location;
            var coroutineLock = await self.Root().GetComponent<CoroutineLockComponent>().Wait(coroutineLockType, key);

            var lockInfo = self.AddChild<LockInfo, ActorId, CoroutineLock>(actorId, coroutineLock);
        ...
        {
            if (!self.lockInfos.TryGetValue(key, out var lockInfoRef))
            {
        ...
        public static async ETTask<ActorId> Get(this LocationOneType self, long key)
        {
            ...
            using (await self.Root().GetComponent<CoroutineLockComponent>().Wait(coroutineLockType, key))
            {
                self.locations.TryGetValue(key, out var actorId);
