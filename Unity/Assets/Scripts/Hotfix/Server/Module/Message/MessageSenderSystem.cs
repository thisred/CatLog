...
namespace ET.Server
{
    ...
    public static partial class MessageSenderSystem
    {
        public static void Send(this MessageSender self, ActorId actorId, IMessage message)
        {
            ...
            // 发给NetInner纤程
            var a2NetInnerMessage = A2NetInner_Message.Create();
        ...
        )
        {
            ...
            {
                // 发给NetInner纤程
                var a2NetInner_Request = A2NetInner_Request.Create();
                a2NetInner_Request.ActorId = actorId;
                a2NetInner_Request.MessageObject = request;
            
                using var a2NetInnerResponse = await fiber.Root.GetComponent<ProcessInnerSender>().Call(
