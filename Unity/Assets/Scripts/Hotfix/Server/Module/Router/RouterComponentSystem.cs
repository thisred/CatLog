...
namespace ET.Server
{
    ...
    public static partial class RouterComponentSystem
    {
        ...
        {
            var ipEndPoint = (IPEndPoint) self.IPEndPoint;
        ...
        private static void CheckConnectTimeout(this RouterComponent self, long timeNow)
        {
            ...
            for (int i = 0; i < n; ++i)
            {
                ...
                var node = self.GetChild<RouterNode>(id);
                if (node == null)
                {
                    continue;
                }
        ...
        private static void RecvOuterHandler(this RouterComponent self, int messageLength, long timeNow, IKcpTransport transport)
        {
            ...
            switch (flag)
            {
                case KcpProtocalType.RouterReconnectSYN:
                {
                    ...
                    var routerNode = self.GetChild<RouterNode>(outerConn);
                    if (routerNode == null)
                    {
        ...
        {
            var routerNode = self.AddChildWithId<RouterNode>(outerConn);
            routerNode.InnerConn = innerConn;
        ...
        {
            var routerNode = self.GetChild<RouterNode>(id);
            if (routerNode == null)
            {
                return;
            }
