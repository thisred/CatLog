        ...
        private async ETTask HandleAsync(NetComponentOnRead args)
        {
            ...
            switch (message)
            {
                ...
                case IRoomMessage actorRoom:
                {
                    ...
                    var roomActorId = player.GetComponent<PlayerRoomComponent>().RoomActorId;
                ...
                case ILocationRequest actorLocationRequest: // gate session收到actor rpc消息，先向actor 发送rpc请求，再将请求结果返回客户端
                {
                    ...
                    var iResponse = await root.GetComponent<MessageLocationSenderComponent>().Get(LocationType.Unit).Call(unitId, actorLocationRequest);
